...
var questionsAnswersArray; // questions and answers array
var currentQuestion;
var answer;
var trueKey;
var falseKey;
...
questionsAnswersArray =
 [
   ["The internet is part of the world wide web", false],
   ["The world wide web is par of the internet", true ],
   ["127.0.0.1 is localhost", true],
...
function create() {
  // constructor new TileSprite(game, x, y, width of sprite, height of sprite, key of image used, frame is part of sprite used)
    ...
    trueKey = game.input.keyboard.addKey(Phaser.Keyboard.T);
    falseKey = game.input.keyboard.addKey(Phaser.Keyboard.F);
...
function update() {
  ...
  if(enterKey.isDown && water.y == startPosition) {
   ...
   if (runningGame) {
     // get array index, show question on screen and then check for correct key input
    ...
    //  if(cursors.down.isDown){
      ...
      else if (falseKey.isDown) {
        answer = false;
      }

    if(answer == questionsAnswersArray[currentQuestion][1]){
      water.y+=100;
      upDateQuestion();
    } else if (answer != questionsAnswersArray[currentQuestion][1]) {
      water.y-=10;
    }
     else  {
      water.y-=.4;
...
function upDateQuestion(){
  ...
  currentQuestion = Math.floor(Math.random() * (questionsAnswersArray.length));
  if(currentQuestion == lastQuestion) {
    upDateQuestion();
    console.log(currentQuestion);

  } else {
    text.text = questionsAnswersArray[currentQuestion][0];
  }
